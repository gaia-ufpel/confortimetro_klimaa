...
const DATE_PATCHER = (props: any) => {
    ...
    async function decideSignalToFetchMetrics(start_datetime:string, end_datetime:string) {
        evaluatePeriod(start_datetime, end_datetime)
        if (toggleWarningPeriod == false){
            return console.log("Enviar sinal")
        }
    }

    return (
        <div className='relative flex flex-col items-center text-center'>
            <button className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded" onClick={() => { decideSignalToFetchMetrics(props.start_datetime, props.end_datetime)}}>Monitorar</button>
